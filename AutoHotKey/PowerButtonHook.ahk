...
WM_POWERBROADCAST(wParam,lParam) {
	...
	if(wParam=PBT_POWERSETTINGCHANGE){
		if(subStr(strGet(lParam),1,strLen(strGet(lParam))-1)=strGet(&newGUID)){
			...
			RunWait taskkill /F /IM explorer.exe ; Restarting explorer will re-enable the display
			sleep 2000
			Run explorer.exe
			sleep 8000 ; To prevent AV receiver and TV from being switched on and off too much and becoming out of sync, remove if not needed
		}
	}
	return
}
